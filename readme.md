...
## 十一、外设驱动篇
&emsp;&emsp;这一部分内容主要聚焦于外设驱动的开发与应用，虽然与操作系统的核心设计关系不大，但它在嵌入式系统的实际应用中扮演着至关重要的角色。外设驱动是连接软件与硬件的桥梁，驱动的编写质量和效率直接影响到系统对硬件资源的利用能力和整体运行性能。在这里，将介绍一些常见外设的基本操作方式，掌握如何初始化这些外设，如何处理其输入输出、以及如何处理中断等。
### 11.1、ADC
&emsp;&emsp;ADC是一种将模拟信号（如电压、电流）转换为数字信号的电子器件，在嵌入式系统中被广泛用于采集来自外部世界的物理量。ADC模块的基本工作原理是通过采样保持电路对模拟信号进行周期性采样，并在指定的采样精度下将其转换为对应的数字值。例如，一个12位ADC可以将输入的模拟电压分成4096个离散等级，并输出相应的数字编码。在嵌入式系统中使用ADC，通常要完成以下几个步骤：
1. ✅ **初始化配置**：设置 ADC 通道、采样精度（如 8 位、10 位、12 位）、采样时间、触发方式（软件触发或外部触发）等参数。
2. ▶️ **启动采样**：根据配置选择启动方式，如单次转换（只采样一次）、连续转换（循环采样）、扫描模式（多通道依次采样）。
3. 📥 **读取结果**：转换完成后，从数据寄存器中读取数字结果，部分平台支持通过中断或 DMA 自动读取数据。
4. 🔧 **数据处理**：对获得的数字信号进行滤波、校准、单位换算等处理，以便后续逻辑使用，如温度判断、电压检测等。

&emsp;&emsp;**详见07-ADC-DMA。**

### 11.2、DMA
&emsp;&emsp;DMA是一种允许外设或内存模块在无需CPU参与的情况下，直接进行数据传输的硬件机制。通过DMA可以在后台高效完成大批量数据的搬运任务，从而显著减轻CPU负担，提高系统整体性能。在嵌入式系统中，DMA常用于以下场景：ADC采样结果自动传输到内存、串口收发数据、SPI/I2C传输缓存等，特别适合数据量大、传输频繁的应用。
DMA 使用流程如下：
1. ⚙️ **初始化配置**：配置 DMA 通道、源地址、目标地址、传输方向（如外设到内存）、数据宽度、传输大小、是否使用中断等。
2. 🚀 **启动传输**：使能 DMA 通道，系统开始按照设定自动进行数据搬运，无需 CPU 参与。
3. 🛎️ **传输完成处理**：可选择在传输完成、中途出错等事件触发中断，由中断服务函数处理相应逻辑，如重新配置 DMA、通知主程序等。
4. 🔁 **循环/链式模式（可选）**：某些应用中需要 DMA 持续运行或处理多个传输任务，可启用循环模式或链式传输机制（如双缓冲）。

&emsp;&emsp;在这里DMA结合ADC实现高速采样功能，**详见07-ADC-DMA**。